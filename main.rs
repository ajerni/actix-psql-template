...
pub struct CreateAaabbbRecordRequest {
    pub name: String,
    pub age: i32,
}

// CRUD operations
async fn create_aaabbb(
    pool: web::Data<PgPool>,
    record: web::Json<CreateAaabbbRecordRequest>,
    ...
    let query = format!("INSERT INTO aaabbb_table ({}) VALUES ({}) RETURNING *", fields_str, values_str);

    let result = sqlx::query_as::<_, AaabbbRecord>(&query)
        .bind(&record.name)
        .bind(&record.age)
        .fetch_one(pool.get_ref())
        .await;
...
async fn get_aaabbb(
    ...
    let result = sqlx::query_as::<_, AaabbbRecord>(
        "SELECT * FROM aaabbb_table WHERE id = $1"
    )
    .bind(id.into_inner())
...
async fn list_aaabbb(
    pool: web::Data<PgPool>,
) -> Result<impl Responder> {
    let result = sqlx::query_as::<_, AaabbbRecord>(
        "SELECT * FROM aaabbb_table ORDER BY id"
    )
...
async fn update_aaabbb(
    ...
    let query = format!("UPDATE aaabbb_table SET {} WHERE id = $1 RETURNING *", set_clause);
...
async fn delete_aaabbb(
    pool: web::Data<PgPool>,
    id: web::Path<i32>,
) -> Result<impl Responder> {
    let result = sqlx::query("DELETE FROM aaabbb_table WHERE id = $1")
        .bind(id.into_inner())
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .service(
                ...
                .route("/aaabbb", web::post().to(create_aaabbb))
                .route("/aaabbb", web::get().to(list_aaabbb))
                .route("/aaabbb/{id}", web::get().to(get_aaabbb))
                .route("/aaabbb/{id}", web::put().to(update_aaabbb))
                .route("/aaabbb/{id}", web::delete().to(delete_aaabbb))
            )
