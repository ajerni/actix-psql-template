...
async fn main() -> std::io::Result<()> {
    ...
    // From within Docker network, use service name: actix-server-postgres
    // From host machine, use: localhost:5433
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgresql://actix_user:actix_password@actix-server-postgres:5432/actix_db".to_string());
